...
export const useAssetsStore = create<AssetsState>((_set, get) => ({
  ...
  getAssetById: ((assetId: AssetId | undefined) =>
    assetId ? get().byId[assetId] : undefined) as AssetsState['getAssetById'],
  getAllAssets: () => get().ids.map(id => get().byId[id]),
}))

export const useAssetById = <T extends AssetId | undefined>(
  assetId: T,
): T extends AssetId ? Asset : undefined => {
  return useAssetsStore(state => state.getAssetById(assetId)) as T extends AssetId
    ? Asset
    : undefined
}
